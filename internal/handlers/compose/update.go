...
func UpdateHandler(params UpdateHandlerParams) error {
	...
	// Only persist and reload when the pre-run script actually produced output.
	// An empty result means no script ran (or it emitted nothing); writing it
	// would truncate values.yaml and wipe the existing config.
	if runOutput.Len() > 0 {
		...
		// Reload the existing config from the migrated values so the smart merge
		// below operates on the pre-run script's output rather than discarding it.
		confFile, err = models.CreateFile(configPath, "values.yaml")
		if err != nil {
			logger.Error(err)
			return err

...
func compareFolderContent(folder1 map[string]string, folder2 map[string]string) error {
	...
	for name, content1 := range folder1 {
		...
		content1Trimmed := strings.TrimSuffix(content1, "\n")
		content2Trimmed := strings.TrimSuffix(content2, "\n")

		content1withoutSeed := RemoveLinesWithSeedAndConfigPath(content1Trimmed)
		content2withoutSeed := RemoveLinesWithSeedAndConfigPath(content2Trimmed)

		if content1withoutSeed != content2withoutSeed {
...
func RemoveLinesWithSeedAndConfigPath(input string) string {
	var result []string
	lines := strings.Split(input, "\n")
	for _, line := range lines {
		if !strings.Contains(line, "seed") && !strings.Contains(line, "config: /") {
